...
if __name__ =='__main__':
    ...
    # 2、获取任务中的关键动词和名词，比如take和book
    verb, noun = get_noun_verb(task)

    # 3、状态检测
    img_name = 'image/img3.jpg'
    model_yolov8 = YOLO('ckpt/best.pt')
    results_yolov8 = model_yolov8(img_name)  # results list
    x1, y1, x2, y2, success_flag =state_detect(results_yolov8=results_yolov8, task=task, noun=noun)# 进行物品状态检测，判断当前物品的状态是否适合完成该任务，success_flag = 1 表示适合，success_flag = 2 表示不适合
    if(success_flag == 1):
        print('当前物品的状态适合完成任务')
    if (success_flag == 2):
        print('当前物品的状态不适合完成任务')

    # 经过物品搜寻后，找到了一个状态适合完成任务的物品
